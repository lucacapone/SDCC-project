...
La pipeline host raccoglie i log Compose e opera completamente offline. Il valore atteso deriva soltanto dagli `initial_value` delle configurazioni realmente selezionate per la run; non entra nel processo nodo e non attraversa il protocollo. Gli stessi file forniscono l'insieme esplicito dei `node_id` attesi: prima di calcolare la convergenza, la pipeline lo confronta con le serie osservate e rifiuta sia nodi mancanti sia nodi estranei alla configurazione selezionata. Il raggruppamento resta dinamico e non codifica la cardinalità del cluster.
