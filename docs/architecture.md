...
Sul piano runtime la failure detection viene integrata direttamente dentro `internal/gossip/engine.go`: a ogni tick del round gossip l'engine applica `Membership.ApplyTimeoutTransitions(time.Now().UTC())` prima di selezionare i target, così i peer inattivi degradano automaticamente `alive -> suspect -> dead` senza supporto esterno dei test. Ogni messaggio gossip valido viene inoltre trattato come heartbeat implicito del suo `origin_node`, promuovendo eventuali alias seed `host:port` alla forma canonica `node_id` quando il digest del mittente espone lo stesso `addr` e impedendo che placeholder obsoleti restino peer separati. Le transizioni osservate vengono emesse come log strutturati con evento stabile `membership_transition`, utile per debugging e osservabilità operativa.
