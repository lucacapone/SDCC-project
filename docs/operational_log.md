...
## 2026-04-20 16:01:19 UTC
- **Descrizione task**: Attivazione fanout runtime nel gossip engine con selezione target senza duplicati, allineamento wiring `cmd/node`, test dedicati e aggiornamento documentazione.
- **File modificati**: `internal/gossip/engine.go`, `cmd/node/main.go`, `tests/gossip/engine_test.go`, `tests/gossip/membership_convergence_test.go`, `tests/gossip/integration_test.go`, `tests/gossip/engine_transport_contract_test.go`, `tests/integration/harness_test.go`, `tests/aggregation/sum/sum_convergence_test.go`, `tests/aggregation/average/average_convergence_test.go`, `tests/aggregation/min/min_convergence_test.go`, `tests/aggregation/max/max_convergence_test.go`, `README.md`, `docs/architecture.md`, `docs/configuration.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho esteso `Engine` con `Fanout` e sorgente random iniettabile per test deterministici, normalizzando i valori non positivi a `1`; nel round il filtro membership resta invariato (`selectGossipTargets`) e viene seguito dalla selezione fanout (`pickFanoutTargets`) con invio a tutti quando il fanout supera i peer eleggibili. Ho aggiornato tutte le call-site di `NewEngine`, aggiunto test unitari specifici per i casi richiesti (`fanout=1`, `fanout>peer`, `fanout<=0`) e riallineato README/architettura/configurazione rimuovendo i riferimenti al fanout non applicato.

## 2026-04-20 16:21:18 UTC
...
## 2026-04-20 17:58:11 UTC
- **Descrizione task**: Implementazione del percorso di leave announcement volontario, wiring shutdown orchestrato con annuncio leave pre-stop, aggiunta test di integrazione join/leave e aggiornamento documentazione lifecycle/tempi attesi.
- **File modificati**: `internal/gossip/engine.go`, `cmd/node/main.go`, `tests/integration/harness_test.go`, `tests/integration/node_leave_test.go`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho introdotto `Engine.AnnounceLeave` per marcare il nodo locale in `leave` con `incarnation` incrementata e inviare un annuncio best-effort ai peer prima della chiusura del transport; ho integrato la chiamata nello shutdown di `cmd/node/main.go`; ho esteso l’harness in-memory con metriche di delivery e bootstrap parametrico membership per testare due scenari: convergenza del cluster residuo dopo leave volontario e assenza di targeting del nodo uscito oltre la finestra temporale di protocollo (propagazione + prune); infine ho aggiornato architettura e testing per esplicitare lifecycle join/leave e relativi limiti temporali osservabili.

## 2026-04-22 13:12:40 UTC
- **Descrizione task**: Introduzione test di integrazione Compose scale a 6 nodi con harness parametrico (file Compose + servizi), criteri misurabili di convergenza/osservabilità round-merge e aggiornamento documentazione operativa dedicata.
- **File modificati**: `tests/integration/cluster_convergence_scale_compose_test.go`, `tests/integration/compose_harness_test.go`, `tests/integration/cluster_convergence_test.go`, `tests/integration/node_crash_restart_compose_test.go`, `scripts/cluster_common.sh`, `docs/testing.md`, `docs/deployment.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho esteso l'harness Compose per supportare override del deployment (`SDCC_COMPOSE_FILE`, `SDCC_PROJECT_NAME`, `SDCC_SERVICES`) mantenendo retrocompatibilità con i test reali esistenti; ho aggiunto una suite scale Compose che usa `deploy/docker-compose.scale.yml` e valida convergenza tramite banda e timeout, più segnali osservabili (`sdcc_node_rounds_total`, `sdcc_node_remote_merges_total`) prima del teardown con artefatti finali, allineando i comandi canonici in testing/deployment.

## 2026-04-22 13:21:17 UTC
- **Descrizione task**: Estensione test integrazione in `tests/integration/` per scenario combinato con crash di 2 nodi in sequenza, partizione temporanea di rete e recovery con rejoin; introduzione wrapper/azioni fault injection dedicate; timeout configurabili via env; aggiornamento documentazione testing/demo con criteri di successo e limiti ambientali.
- **File modificati**: `tests/integration/sequential_crash_partition_rejoin_compose_test.go`, `tests/integration/compose_harness_test.go`, `scripts/fault_injection/network_partition.sh`, `scripts/fault_injection/scenario_sequential_crash_partition_rejoin.sh`, `docs/testing.md`, `docs/demo.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho riusato la toolchain di fault injection esistente aggiungendo script mirati per partizione rete e scenario combinato, poi ho creato una suite Compose reale che valida operatività del cluster residuo, convergenza entro banda e membership post-rejoin tramite metriche runtime (`known_peers`). Ho reso configurabili i timeout dello scenario esteso con variabili ambiente per ambienti lenti/CI e ho documentato esplicitamente prerequisiti/limiti legati a Docker daemon e policy di rete.

## 2026-04-22 13:39:03 UTC
- **Descrizione task**: Refactor Bash 3.2-safe di `parse_services_list` negli script cluster, rimozione uso `local -n` (nameref), aggiornamento call-site e nota di compatibilità nel compose harness Go.
- **File modificati**: `scripts/cluster_common.sh`, `tests/integration/compose_harness_test.go`, `docs/operational_log.md`.
- **Reasoning summary**: Ho sostituito la firma della funzione per popolare direttamente l'array globale `SERVICES`, mantenendo invariata la semantica di parsing (separatori spazio/virgola, trim, scarto token vuoti) e il fallback canonico ai tre servizi default. Ho aggiornato i punti d'uso coerentemente, verificato che il messaggio d'errore resti invariato e aggiunto una nota esplicita sulla compatibilità Bash 3.2 nel test harness che invoca gli script con `exec.Command("bash", ...)`.

## 2026-04-25 12:58:11 UTC
- **Descrizione task**: Hardening merge gossip per `sum` con stato canonico per-contributo idempotente, tie-break deterministico su stessa versione/payload diverso, logging diagnostico mirato e aggiornamento test/documentazione.
- **File modificati**: `internal/gossip/state.go`, `internal/gossip/engine.go`, `tests/aggregation/sum/sum_convergence_test.go`, `tests/gossip/engine_test.go`, `docs/architecture.md`, `docs/observability.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho reso il merge `sum` sempre per-chiave (`node_id`) basato su `aggregation_data.sum.contributions` + `versions`, eliminando il percorso che poteva trattare il totale remoto come base incrementale; `estimate` viene ora mantenuto come valore derivato dalla somma dei contributi, con contributo locale stabile tramite `LocalValue` in round. In caso di stessa versione ma payload differente per lo stesso nodo, la risoluzione è deterministica (`tie_break` stabile), con tracciamento decisioni (`newer_version`, `tie_break`, `duplicate_ignored`) e nuovi campi log (`unique_nodes`, `estimate_before/after`, `conflict_node_id`, `conflict_decision`). Ho esteso la suite `sum` con scenario a 6 nodi, replay/out-of-order e conflitto stessa versione; infine ho aggiornato architettura, testing e observability per allineare semantica idempotente e lettura dei log di conflitto.

## 2026-04-25 13:40:37 UTC
- **Descrizione task**: Refactor merge gossip per separare classificazione stato da merge numerico, con regola deterministica dedicata a `aggregation=max` nei conflitti a stessa versione/payload differente e arricchimento log diagnostico (`max_preserved`).
- **File modificati**: `internal/gossip/state.go`, `internal/gossip/engine.go`, `tests/gossip/state_test.go`, `tests/gossip/engine_test.go`, `docs/architecture.md`, `docs/observability.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho introdotto una fase esplicita di decisione (`resolveMergeDecision`) per separare la logica di stato (`applied/skipped/conflict`) dalla logica numerica del merge. Per `max`, nel ramo `same_version_different_payload` il valore post-merge viene sempre calcolato come `max(estimate_before, remote_estimate)` senza regressioni dovute a tie-break non numerici; il risultato espone `MaxPreserved` in `MergeResult` e il logger `remote_merge` include `max_preserved` per facilitare debug. Ho aggiunto test su conflitto `max` per commutatività/idempotenza e aggiornato la documentazione architetturale/osservabilità per mantenere allineamento con il comportamento runtime.

## 2026-04-25 13:47:12 UTC
...
## 2026-05-14 08:25:33 UTC
- **Descrizione task**: Implementazione eleggibilita' membership-based per il calcolo locale di `sum` e `average`.
- **File modificati**: `internal/membership/membership.go`, `internal/gossip/engine.go`, `tests/membership/membership_test.go`, `tests/membership/reexport.go`, `tests/gossip/engine_test.go`, `tests/gossip/reexport.go`, `tests/aggregation/average/average_convergence_test.go`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Ho centralizzato la regola di eleggibilita' in membership, ammettendo solo peer `alive` e mantenendo il self node eleggibile durante il bootstrap quando non e' ancora nello snapshot. Nel round gossip ho costruito il set dei nodi eleggibili dalla membership corrente e ho ricalcolato `sum`/`average` filtrando solo `state.value`, senza cancellare contributi o versioni dai metadata CRDT-like. Ho aggiunto regressioni su stati non eleggibili, self bootstrap e conservazione dei contributi esclusi, aggiornando documentazione architetturale e testing per riflettere la nuova semantica.

## 2026-05-14 08:33:56 UTC — Ricalcolo aggregato membership-aware

- **Descrizione task**: reso il ricalcolo di `state.value` esplicitamente dipendente dallo snapshot membership corrente nei round locali e nei merge remoti.
- **File modificati**: `internal/gossip/engine.go`, `tests/gossip/engine_test.go`, `tests/gossip/concurrency_test.go`, `tests/aggregation/sum/sum_convergence_test.go`, `tests/aggregation/average/average_convergence_test.go`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: dopo aver riletto documentazione, configurazioni, testing e log operativo, ho spostato la stima osservabile post-merge su un ricalcolo eseguito dopo `markPeerAlive` e `mergeMembership`, usando lo snapshot aggiornato senza cancellare contributi storici. Ho inoltre reso `prepareLocalStateForRound` dipendente da `selfID` e dallo snapshot membership, aggiornato collector/log a usare la stima ricalcolata, aggiunto regressioni mirate e riallineato harness deterministici affinché modellino una membership `alive` esplicita coerente con la nuova semantica.

## 2026-05-14 08:44:48 UTC
- **Descrizione task**: Estensione di `min` e `max` al ricalcolo membership-aware basato su contributi per nodo.
- **File modificati**: `internal/types/types.go`, `internal/gossip/engine.go`, `internal/gossip/state.go`, `tests/gossip/engine_test.go`, `tests/aggregation/min/min_convergence_test.go`, `tests/aggregation/max/max_convergence_test.go`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Dopo la rilettura di documentazione, configurazioni, testing e log operativo, ho allineato `min` e `max` alla semantica CRDT-like gia' usata da `sum`/`average`: ogni nodo mantiene un contributo scalare versionato, i merge fondono contributi per `node_id` usando versioni e tie-break deterministici, e il valore osservabile viene ricalcolato sulla membership eleggibile senza cancellare contributi storici. Il fallback senza contributori eleggibili resta `0`, mentre nei round locali il contributo self viene registrato prima del calcolo se eleggibile. Ho aggiornato regressioni e documentazione per coprire esclusione di nodi non `alive`, conservazione dei metadata e payload min/max con contributi espliciti.

## 2026-05-14 08:51:03 UTC
- **Descrizione task**: Introduzione della policy status-level riusabile per stabilire l'eleggibilita' dei nodi al calcolo aggregato.
- **File modificati**: `internal/membership/membership.go`, `internal/gossip/engine.go`, `tests/membership/membership_test.go`, `tests/membership/reexport.go`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Dopo aver riletto documentazione, configurazioni, testing e log operativo, ho estratto la regola esplicita `IsAggregationEligible(status)` nel package membership: solo `Alive` e' ammesso, mentre `Suspect`, `Dead`, `Left`, stato vuoto e stati non riconosciuti sono esclusi. L'engine gossip usa ora questa policy condivisa nella costruzione del set di contributori aggregati, mantenendo separato il caso bootstrap self gia' gestito nel motore. Ho aggiornato re-export e regressioni membership per congelare la policy a livello di `Status`, oltre alla documentazione collegata.

## 2026-05-14 12:29:32 UTC
- **Descrizione task**: Aggiornamento test per la semantica di esclusione dei nodi non attivi dagli aggregati per contributo.
- **File modificati**: `tests/integration/node_crash_restart_test.go`, `tests/integration/harness_test.go`, `tests/aggregation/average/average_convergence_test.go`, `tests/aggregation/sum/sum_convergence_test.go`, `tests/aggregation/min/min_convergence_test.go`, `tests/aggregation/max/max_convergence_test.go`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: Dopo la rilettura di documentazione, configurazioni, testing e log operativo, ho allineato la variante crash/restart in-memory affinché il cluster residuo usi come riferimento la media dei soli nodi rimasti `alive`. Ho aggiunto uno scenario esplicito a 6 nodi con valori `10, 30, 50, 70, 90, 110`, verificando media iniziale `60`, media `40` dopo `leave`/`dead` di `node-5`/`node-6` e ritorno a `60` dopo rejoin `alive`. Ho inoltre aggiunto casi black-box senza rete reale per `average`, `sum`, `min` e `max`, controllando filtro membership, conservazione dei contributi storici e riattivazione dopo rejoin.

## 2026-05-14 12:36:08 UTC
- **Descrizione task**: Aggiornamento documentazione del cambio comportamento membership-aware sugli aggregati esposti.
- **File modificati**: `README.md`, `docs/architecture.md`, `docs/configuration.md`, `docs/testing.md`, `docs/task/M05.md`, `docs/task/M10.md`, `docs/operational_log.md`.
- **Reasoning summary**: Dopo la rilettura di documentazione, configurazioni, testing e log operativo, ho aggiornato solo documentazione pertinente per rendere esplicita la separazione tra stato gossip conservato, vista membership locale e risultato aggregato esposto. La decisione documentata è che i contributi dei nodi `suspect`, `dead` e `leave` restano disponibili nei metadata per recovery/rejoin ma sono esclusi da `state.value`, metriche e log finché la membership locale non li considera nuovamente `alive`; i timeout esistenti `SuspectTimeout`, `DeadTimeout` e `PruneRetention` determinano il passaggio di eleggibilità senza introdurre nuove soglie hardcoded.

## 2026-05-18 12:23:18 UTC
...
## 2026-05-20 10:04:15 UTC
- **Descrizione task**: Introduzione policy di emissione log `event=remote_merge` basata su significatività, con modalità configurabili (`full|significant|off`) e soglia delta stima configurabile.
- **File modificati**: `internal/gossip/engine.go`, `internal/config/config.go`, `cmd/node/main.go`, `configs/example.yaml`, `docs/configuration.md`, `tests/gossip/engine_test.go`, `docs/operational_log.md`.
- **Reasoning summary**: Ho mantenuto il comportamento runtime controllabile via config senza ricompilazione aggiungendo due nuove opzioni (`logging.remote_merge_mode`, `logging.log_estimate_delta_threshold`), applicate nel bootstrap del nodo all'engine gossip. L'emissione INFO ora è guidata da `merge_significant` (status non applied, cambi aggregazione/membership o delta stima oltre soglia), con soppressione dei merge non significativi in modalità `significant` e spegnimento completo in `off`; ho aggiornato test e documentazione configurazione coerentemente.

## 2026-09-08 13:25:04 UTC — Pipeline passiva di convergenza CSV/SVG

- **Obiettivo**: aggiungere campionamento passivo della stima gossip e una pipeline host riproducibile che conservi i log Compose, generi un CSV normalizzato, calcoli offline il riferimento dalle configurazioni della run e produca grafico SVG e riepilogo per topologie dinamiche.
- **File modificati**: `README.md`, `cmd/convergence-chart/main.go`, `docs/architecture.md`, `docs/demo.md`, `docs/deployment.md`, `docs/observability.md`, `docs/testing.md`, `docs/operational_log.md`, `internal/convergence/convergence.go`, `internal/convergence/convergence_test.go`, `internal/gossip/engine.go`, `scripts/cluster_convergence_report.sh`, `tests/gossip/engine_test.go`.
- **Reasoning summary**: dopo la rilettura obbligatoria di documentazione, architettura, configurazioni, testing, note operative, task report e storico operativo, ho mantenuto invariati calcolo aggregativo, membership, selezione peer e wire protocol. L'engine emette una fotografia iniziale prima di aprire il transport, un punto per round locale e un punto dopo variazioni significative da merge. Il comando offline ordina e normalizza rispetto al primo campione globale, ricava `sum`/`average`/`min`/`max` soltanto dagli `initial_value`, costruisce serie a gradini per tutti i `node_id` osservati e identifica il primo suffisso interamente entro tolleranza, gestendo esplicitamente l'assenza di convergenza. Lo script riusa `cluster_common.sh`, forza la ricreazione dei container per delimitare la run e scrive solo negli artefatti host già esclusi da Git; la validazione Compose richiesta non è stata eseguibile perché il comando `docker` non è installato nell'ambiente, mentre suite Go, vet, sintassi shell e controlli diff sono stati eseguiti localmente.

## 2026-09-08 15:56:41 UTC — Etichette adattive per la griglia SVG di convergenza

- **Descrizione task**: Aggiunte etichette temporali in secondi alle linee verticali ed etichette della stima alle linee orizzontali del grafico SVG, con precisione adattiva e regressioni sugli estremi e su tacche intermedie.
- **File modificati**:
...
## 2026-09-15 12:22:08 UTC — Generation durevole per restart/rejoin della stessa NodeID

- **Descrizione task**: Correzione del rejoin PRE-TC tramite generation locale persistente, condivisa da `VersionEpoch` e membership `Incarnation`, senza indebolire `out_of_order_stale`.
- **File modificati**: `Dockerfile`, `README.md`, `cmd/node/main.go`, `docker-compose.yml`, `deploy/docker-compose.scale.yml`, `docs/architecture.md`, `docs/configuration.md`, `docs/deployment.md`, `docs/deployment_ec2.md`, `docs/task/M10.md`, `docs/testing.md`, `internal/gossip/engine.go`, `internal/gossip/state.go`, `internal/membership/membership.go`, `tests/aggregation/average/average_convergence_test.go`, `tests/aggregation/max/max_convergence_test.go`, `tests/aggregation/min/min_convergence_test.go`, `tests/aggregation/sum/sum_convergence_test.go`, `tests/gossip/engine_test.go`, `tests/gossip/membership_convergence_test.go`, `tests/gossip/state_test.go`, `tests/integration/cluster_convergence_test.go`, `tests/integration/harness_test.go`, `tests/integration/node_crash_restart_test.go`, `tests/membership/membership_test.go`; nuovi `internal/identity/generation.go` e `tests/identity/generation_test.go`.
- **Reasoning summary**: Dopo la rilettura obbligatoria di documentazione, architettura, configurazioni, testing, note operative, task report e storico, ho introdotto un allocatore fail-fast con lock, file temporaneo, fsync e rename atomico. Il boot assegna la stessa generation a epoch gossip e incarnation self, il leave conserva tale incarnation, i digest ordinari includono il self e gli heartbeat non resuscitano `dead/leave` senza generation superiore. L'epoch locale non adotta più epoch remote. Volumi distinti per nodo preservano la generation; la perdita tramite `down -v` resta esplicitamente non recuperata dai peer. Le regressioni coprono storage, versioni stale cross-epoch, membership monotona e la sequenza stabile a sei nodi `60 -> 50 -> 40 -> 50 -> 60` senza `Touch` di rejoin.

## 2026-09-15 15:13:35 UTC — Fotografia istantanea dello stato aggregativo Compose

- **Descrizione task**: Creato lo script manuale `scripts/show_aggregation_status.sh` per leggere una sola volta i servizi running del progetto Compose `sdcc-scale`, estrarre da ciascun log l'ultimo `event=convergence_sample` valido e mostrare stime e coerenza dell'aggregazione senza polling, attese, processi persistenti o ricalcolo centralizzato.
- **File modificati**:
  - `scripts/show_aggregation_status.sh` (creato ed eseguibile)
  - `docs/testing.md`
  - `docs/operational_log.md`
- **Reasoning summary**: Dopo la rilettura obbligatoria di documentazione, architettura, configurazioni, testing, note operative, report di milestone e storico operativo, ho interrogato dinamicamente Compose con `ps --services --status running` e isolato la lettura best-effort dei log per servizio. Lo script valida `node_id`, le sole aggregazioni supportate (`average`, `sum`, `min`, `max`) e una stima numerica, presenta `N/D` e stato non zero per campioni assenti o illeggibili e mostra la colonna aggregazione soltanto in caso di discordanza. Le prove con un comando Docker simulato hanno coperto sei nodi running, arresto e riattivazione di node6, discordanza, dato mancante ed uscita entro due secondi; la verifica Compose reale non è stata possibile perché Docker non è installato nell'ambiente. Sono state inoltre eseguite la suite Go, `go vet`, la validazione sintattica Bash e il controllo whitespace del diff.

## 2026-09-16 13:40:29 UTC — Modalità sperimentale Traffic Control isolata
- **Descrizione task**: implementazione ex novo della modalità opzionale a sei nodi con Linux `tc/netem`, secondo workflow task/stub/activities, senza recuperare implementazioni storiche e senza modificare il percorso normale.
- **File creati**: `deploy/traffic-control/Dockerfile`, `deploy/traffic-control/entrypoint.sh`, `deploy/docker-compose.tc.yml`, `scripts/demo_tc_latency.sh`, `docs/traffic_control.md`, `docs/task/TC01.md`.
- **File modificati**: `README.md`, `docs/architecture.md`, `docs/testing.md`, `docs/operational_log.md`.
- **Reasoning summary**: l'esperimento costruisce lo stesso `cmd/node` in un'immagine Alpine dedicata con shell, curl e iproute2; il solo Compose TC assegna `NET_ADMIN`, usa project/rete/volumi separati e conserva i YAML applicativi. L'entrypoint valida delay/jitter, ricava l'interfaccia dalla route verso un peer, usa `tc qdisc replace`, verifica la qdisc (oppure il bypass di node1) ed esegue il nodo con `exec`. La demo usa oracle statici, metriche su loopback per `known`, log `convergence_sample` per la stima, epsilon `0.000001`, timeout 30s e invalidazione immediata per false suspicion a container attivi. Sono risultati positivi `go test ./...`, `go vet ./...`, `bash -n`, self-test shell, permessi eseguibili, parsing YAML via Ruby e `git diff --check`; Docker/Compose/NET_ADMIN/NetEm non sono stati eseguiti perché il comando `docker` non è installato nell'ambiente, quindi la validazione runtime resta esplicitamente demandata a Mac/EC2 compatibili.

## 2026-09-16 13:55:10 UTC — Build unica dell'immagine Traffic Control

- **Descrizione task**: corretta esclusivamente la strategia di build della modalità TC per impedire che Docker Compose costruisca ed esporti contemporaneamente sei volte la stessa immagine `sdcc-node-tc:local` prima dello startup.
- **File modificati**: `deploy/docker-compose.tc.yml`, `scripts/demo_tc_latency.sh`, `docs/traffic_control.md`, `docs/task/TC01.md`, `docs/operational_log.md`.
- **Reasoning summary**: la causa confermata era il blocco `build` ereditato dall'anchor da tutti i sei servizi: `docker compose up --build` creava sei target concorrenti con lo stesso tag e poteva fallire durante l'export con `image already exists`. Ho rimosso la responsabilità di build dal Compose TC; lo script ora esegue un singolo `docker build` del Dockerfile TC con tag condiviso e, solo dopo il suo successo, avvia i sei servizi tramite `docker compose up --no-build`. Immagine, binario, configurazioni, profili NetEm, oracle e comportamento runtime restano invariati. Sono risultati positivi `bash -n scripts/demo_tc_latency.sh`, il self-test esistente, il parsing YAML con verifica statica di sei servizi senza `build` e con un unico tag, e `git diff --check`; `docker compose config` e i test Docker runtime non sono stati eseguibili perché il comando `docker` non è installato nell'ambiente.

## 2026-09-16 14:12:22 UTC — Retry bounded del DNS in bootstrap Traffic Control

- **Descrizione task**: resa robusta la sola risoluzione del peer nell'entrypoint TC contro la race di pubblicazione iniziale dei nomi nel DNS Docker, con policy autorizzata di retry ogni 250 ms per un massimo di 10 secondi.
- **File modificati**: `deploy/traffic-control/entrypoint.sh`, `docs/traffic_control.md`, `docs/operational_log.md`.
- **Reasoning summary**: il lookup resta immediato al primo tentativo e, soltanto se fallisce durante il bootstrap, viene ripetuto al massimo 40 volte prima di terminare chiaramente non-zero. Appena disponibile l'IPv4 del peer, route, individuazione dell'interfaccia, applicazione/verifica NetEm ed `exec` del nodo seguono il flusso preesistente. Compose, codice Go, delay/jitter, gossip, membership e timeout della demo restano invariati. La sintassi shell, il self-test TC esistente, un harness con DNS simulato (successo dopo tre fallimenti e timeout bounded), `go vet ./...`, il rerun della suite Go completa e `git diff --check` sono risultati positivi; una prima esecuzione della suite completa ha mostrato una failure temporale nella suite integration ed è passata al rerun mirato. Docker/Compose/NetEm non sono stati eseguiti perché `docker` non è installato nell'ambiente; anche `shellcheck` non è disponibile.

## 2026-09-16 14:20:27 UTC — Quoting della diagnostica peer Traffic Control

- **Descrizione task**: corretta esclusivamente la funzione diagnostica `peer_interface()` della demo TC affinché il peer raggiunga lo shell Alpine interno come argomento posizionale e i riferimenti ai campi awk non vengano espansi preventivamente dalla shell.
- **File modificati**: `scripts/demo_tc_latency.sh`, `docs/operational_log.md`.
- **Reasoning summary**: dopo la rilettura obbligatoria della documentazione, dell'architettura, delle configurazioni, del testing, delle note operative, dei task report e dello storico, ho sostituito i programmi awk tra doppi apici con quoting shell annidato che consegna letteralmente `$1`, `$i` e `$(i + 1)` ad awk. La funzione conserva il peer in `$1`, risolve il primo IPv4 con costrutti awk POSIX, interroga la route e restituisce l'interfaccia; controlli espliciti sugli output e la cattura fail-fast di `ip` mantengono non-zero gli errori reali. Entry point, Compose, profili NetEm e logica applicativa/demo restano invariati. Sintassi Bash, self-test, harness mirato con comandi simulati e `git diff --check` sono risultati positivi; Docker non è installato nell'ambiente, quindi la prova runtime Alpine/Compose resta demandata al Mac.

## 2026-09-16 15:50:06 UTC — Margine e osservazione post-convergenza Traffic Control

- **Descrizione task**: corretto esclusivamente il profilo TC dopo le validazioni AWS EC2 e impedito che la demo dichiari successo prima di una finestra minima di osservazione stabile.
- **File modificati**: `deploy/docker-compose.tc.yml`, `scripts/demo_tc_latency.sh`, `docs/traffic_control.md`, `docs/operational_log.md`.
- **Reasoning summary**: due run AWS distinte avevano mostrato false suspicion di `node6` ancora `Up` dopo `elapsed_ms=5097` su `node1` e `elapsed_ms=5123` su `node2`, nonostante convergenza a `60` e successivi `remote_merge`. Ho quindi applicato il profilo approvato `0/0`, `400/80`, `800/160`, `1200/240`, `1600/320`, `2000/400` ms, mantenendo jitter al 20% e lasciando invariati gossip, membership timeout, codice Go, configurazioni normali e aggregazioni. Il monitor richiede ora almeno 8 secondi dalla propria partenza, ricontrolla a ogni ciclo container, false suspicion e convergenza, ritorna a `[WAIT]` in caso di regressione e conserva il timeout complessivo di 30 secondi. I log sono delimitati dal timestamp acquisito subito prima di `up --force-recreate` e appartengono ai container appena ricreati, evitando eventi storici. Sintassi Bash, self-test e harness deterministici per osservazione, successo non anticipato ed exit non-zero su false suspicion sono risultati positivi; anche `git diff --check` è positivo. Docker non è installato nell'ambiente, quindi `docker compose ... config --quiet` e la validazione runtime NetEm non sono stati eseguiti: la validazione resta aperta e dovrà proseguire prima su Mac e poi su AWS EC2.

## 2026-09-16 16:30:00 UTC — Telemetria della prima convergenza Traffic Control
...
## 2026-09-17 18:02:08 UTC — Risultati definitivi e figure PNG nel report IEEE

- **Descrizione task**: aggiornato il report IEEE con le cinque figure PNG versionate, le quattro run baseline definitive selezionate e le run rappresentative Traffic Control locale e AWS EC2 single-host, preservando la diversa semantica dei rispettivi monitor.
- **File modificati**: `docs/main.tex`, `docs/operational_log.md` (append-only).
- **Reasoning summary**: dopo la lettura obbligatoria della documentazione, dell'architettura, delle configurazioni, del testing, delle note operative, del log e degli artefatti TC, ho sostituito i riferimenti PDF con inclusioni PNG dirette, disposto l'architettura su entrambe le colonne e mantenuto i quattro grafici in griglia 2x2. Results riporta i tempi baseline 2.010/1.069/2.008/1.054 s con campioni e completezza 6/6, qualifica ogni misura come run singola (`n=1`) e aggiunge una tabella compatta per Average. Il testo TC distingue la prima convergenza simultanea dal controllo conclusivo positivo, non interpreta l'intervallo come stabilità continua, descrive NetEm come impairment egress sintetico e ribadisce che AWS usa sei container su una sola EC2. Non sono stati modificati protocollo, Traffic Control, configurazioni o immagini. Le verifiche statiche dei riferimenti, dei valori e della semantica, l'ispezione diretta delle cinque PNG e `git diff --check` sono positive. La compilazione e l'ispezione del PDF non sono state eseguibili perché `pdflatex` e gli strumenti PDF non sono installati; il tentativo di installarli è fallito per il proxy HTTP 403 dei repository APT, quindi numero di pagine, box e resa finale restano non verificabili in questo ambiente.

## 2026-09-17 18:31:32 UTC — Ridenominazione finale del report scientifico

- **Descrizione task**: rinominato il sorgente LaTeX del report finale e aggiornato il README affinché presenti e colleghi il report disponibile, senza alterarne il contenuto scientifico.
- **File modificati**: `docs/main.tex` rinominato in `docs/report_sdcc.tex`, `README.md`, `docs/operational_log.md` (append-only).
- **Reasoning summary**: la ricerca repository-wide ha individuato il nome precedente soltanto nello storico append-only del log operativo; quel riferimento resta intenzionalmente invariato perché descrive una modifica passata. La sezione Project Report usa ora un link Markdown relativo e sintetizza formato e copertura del documento. La Repository Structure indicava soltanto la directory `docs/` e non richiedeva correzioni. Il rename conserva invariati i riferimenti relativi `figures/...`, validi compilando dalla directory `docs/`.
