...
## 2026-09-09 13:48:11 UTC — Harness sperimentale NetEm e confronto convergenza M13

- **Obiettivo**: aggiungere un esperimento EC2 riproducibile a sei nodi che confronti baseline e delay NetEm senza modificare il deployment canonico o l'algoritmo gossip.
- **File modificati**: `Makefile`, `README.md`, `deploy/docker-compose.tc.yml`, `deploy/traffic-control/Dockerfile`, `deploy/traffic-control/entrypoint.sh`, `docs/configuration.md`, `docs/deployment.md`, `docs/deployment_ec2.md`, `docs/observability.md`, `docs/testing.md`, `docs/task/M13.md`, `scripts/cluster_convergence_report.sh`, `scripts/experiments/traffic_control.sh`, `scripts/experiments/compare_convergence_tc.sh`, `tests/traffic-control/traffic_control_test.sh`, `tests/integration/traffic_control_compose_test.sh`, `docs/operational_log.md`.
- **Reasoning summary**: è stato scelto un override separato perché il runtime canonico deve restare distroless/non-root e privo di capability. La variante compila lo stesso `cmd/node`, usa Alpine con `iproute2` e `su-exec`, ottiene soltanto `NET_ADMIN`, determina l'interfaccia dalla route, applica/rimuove idempotentemente NetEm e abbandona root prima del nodo. L'harness valida gli input, costruisce una volta, isola gli artefatti per run, delega la definizione temporale a `cmd/convergence-chart`/`internal/convergence.ConvergenceTime`, verifica topologia/valore/qdisc e rifiuta run con transizioni `suspect`/`dead`. Sono stati aggiunti test con comandi `tc` iniettati e una suite Docker lenta; nessun tempo EC2 è stato dichiarato senza osservazione reale.

## 2026-09-09 13:50:44 UTC — Validazione M13 nell'ambiente di sviluppo

- **Obiettivo**: validare sintassi, unit test traffic-control, suite Go, vet, YAML e integrità dei file canonici prima del commit.
- **File modificati**: `docs/operational_log.md` (solo append della presente evidenza).
- **Reasoning summary**: `make test-traffic-control`, sintassi Bash, parsing YAML con Ruby, `go vet ./...`, `git diff --check` e un rerun completo di `go test ./... -count=1` sono passati. Una prima esecuzione della suite Go ha incontrato il fallimento intermittente preesistente di `TestRoundOnceConcurrentWithRemoteDelivery` (membership vuota), mentre il rerun repository-wide è passato senza modifiche al codice Go. Docker non è installato nell'ambiente corrente, quindi Compose config tramite Docker, qdisc reale, cluster baseline/TC e confronto `RUNS=3` restano correttamente demandati a Linux/EC2; non sono stati registrati tempi fittizi. È stato verificato che `Dockerfile` e `docker-compose.yml` canonici non presentano diff.

## 2026-09-09 15:54:30 UTC — Contratto unico delle evidenze per le run NetEm M13

- **Descrizione task**: Centralizzata in `validate_run_evidence` la validazione delle run baseline/delayed, rendendo `event=gossip_round` diagnostico opzionale e rafforzando fixture, suite Compose e documentazione del contratto sperimentale.
- **File modificati**: `scripts/experiments/compare_convergence_tc.sh`, `tests/traffic-control/traffic_control_test.sh`, `tests/integration/traffic_control_compose_test.sh`, `docs/observability.md`, `docs/testing.md`, `docs/deployment.md`, `docs/deployment_ec2.md`, `docs/task/M13.md`, `docs/operational_log.md`.
- **Reasoning summary**: Dopo la rilettura obbligatoria di documentazione, architettura, report di milestone, configurazioni, testing, note EC2 e storico operativo, la completezza è stata definita sulle evidenze canoniche: summary completo; presenza di log, CSV, SVG e snapshot qdisc; almeno un campione CSV `local_round` con round positivo e un `remote_merge` per ognuno dei sei nodi; assenza di transizioni `suspect`/`dead`. La funzione delega ancora l'asserzione off/on a `scripts/experiments/traffic_control.sh`, senza inferire NetEm dai tempi. Sono rimasti invariati engine gossip, timeout, configurazione gossip e failure detection. Le fixture coprono ogni evidenza incompleta e il test Compose riesamina il nuovo contratto senza cercare NetEm tramite grep. Test shell, sintassi Bash, vet e controllo whitespace sono passati; la prima suite Go completa ha incontrato il fallimento intermittente preesistente di `TestRoundOnceConcurrentWithRemoteDelivery`, mentre il rerun completo è passato. La suite Compose è stata saltata perché Docker non è installato e shellcheck non è disponibile nell'ambiente.

## 2026-09-10 09:56:54 UTC — Configurazioni TC simmetriche e diagnostica eleggibilita' average

- **Descrizione task**: create sei configurazioni dedicate al Traffic Control copiando integralmente i valori runtime canonici e sostituendo soltanto la discovery con i cinque seed remoti; aggiornato il Compose TC e aggiunta la raccolta diagnostica dell'eleggibilita' average senza cambiare il calcolo di convergenza.
- **File modificati**: `configs/traffic-control/node1.yaml`, `configs/traffic-control/node2.yaml`, `configs/traffic-control/node3.yaml`, `configs/traffic-control/node4.yaml`, `configs/traffic-control/node5.yaml`, `configs/traffic-control/node6.yaml`, `deploy/docker-compose.tc.yml`, `scripts/experiments/compare_convergence_tc.sh`, `tests/traffic-control/traffic_control_test.sh`, `tests/integration/traffic_control_compose_test.sh`, `README.md`, `docs/configuration.md`, `docs/deployment.md`, `docs/deployment_ec2.md`, `docs/observability.md`, `docs/testing.md`, `docs/task/M13.md`, `docs/operational_log.md`.
- **Reasoning summary**: tutti i parametri runtime, inclusi `gossip_interval_ms: 1000`, `fanout: 5` e `membership_timeout_ms: 10000`, restano identici alle configurazioni normali. Baseline e delayed usano il medesimo Compose e gli stessi mount, con `TC_ENABLED` quale unica differenza di condizione. L'harness salva ogni `average_eligible_node_ids`, identifica il primo insieme canonico completo e rileva per ciascun nodo sorgente le successive regressioni sotto sei; se una regressione non e' accompagnata da `suspect/dead`, la validazione si arresta mostrando le evidenze, senza workaround o modifiche al core. Test shell, parsing YAML, suite Go completa, vet e controllo whitespace sono passati; Docker non e' disponibile, quindi la suite Compose/NetEm reale non e' stata eseguita. Una prima suite Go completa ha incontrato un fallimento intermittente nella suite integration gia' noto nello storico; il rerun mirato e il successivo rerun completo sono passati senza modifiche al core.
