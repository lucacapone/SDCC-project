...
% I valori seguenti corrispondono alle esecuzioni attualmente
% disponibili. Prima della consegna sostituirli con quelli
% ottenuti dagli ultimi run definitivi.
% =========================================================
...
\begin{abstract}
Questo lavoro presenta la progettazione, l'implementazione e la valutazione di un servizio decentralizzato per l'aggregazione distribuita dei dati basato su protocollo gossip. Il sistema è sviluppato in Go e non utilizza un coordinatore centrale per il calcolo dell'aggregato: ogni nodo mantiene una vista locale della membership e dello stato aggregativo, scambia periodicamente informazioni con gli altri peer tramite UDP e applica regole deterministiche e idempotenti di merge. Sono supportate quattro operazioni di aggregazione: somma, media, minimo e massimo. Il sistema include inoltre failure detection, gestione di join, leave e rejoin, versionamento persistente dello stato e osservabilità tramite log strutturati e metriche. La soluzione è stata valutata mediante test unitari, test d'integrazione e cluster Docker Compose a tre e sei nodi, includendo scenari di crash e successivo rejoin. Una modalità sperimentale basata su Linux Traffic Control e NetEm introduce delay e jitter artificiali per studiare l'effetto della rete sulla convergenza. Il deployment è stato inoltre verificato su una singola istanza AWS EC2. I risultati mostrano la convergenza dei sei nodi agli aggregati attesi e la capacità del sistema di riconvergere in seguito a variazioni della membership e a condizioni di rete degradate.
\end{abstract}
...
Questo progetto realizza un servizio di \emph{gossip-based distributed data aggregation} completamente implementato in Go. Ogni nodo è equivalente agli altri e mantiene localmente sia i contributi necessari al calcolo dell'aggregato sia una vista della membership. Seed statici o un eventuale servizio di join sono utilizzati esclusivamente per il bootstrap; dopo l'avvio, aggregazione, disseminazione dello stato e failure detection procedono tramite comunicazione peer-to-peer.
...
La valutazione considera correttezza dell'aggregazione, convergenza, comportamento in presenza di crash, riconvergenza dopo il rejoin e sensibilità ai ritardi di rete. Il sistema viene eseguito tramite Docker Compose sia localmente sia su una singola istanza AWS EC2. Una modalità sperimentale separata utilizza Linux \texttt{tc/netem} per introdurre delay e jitter artificiali senza modificare l'algoritmo applicativo.
...
Tutti i nodi eseguono lo stesso processo Go e sono logicamente equivalenti. La presenza di seed peer o di un endpoint di join influenza soltanto il bootstrap e non introduce un nodo responsabile della computazione globale.

\IfFileExists{figures/architecture.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/architecture.pdf}
    \caption{Architettura logica del sistema. Ogni nodo mantiene stato aggregativo e membership e comunica direttamente con gli altri peer tramite gossip UDP.}
...
Il parametro esterno \texttt{membership\_timeout\_ms} viene trasformato in due soglie:
\begin{equation}
T_{suspect}=\max(1\text{ ms},T_m/2),
\end{equation}
\begin{equation}
T_{dead}=\max(T_{suspect}+1\text{ ms},T_m),
\end{equation}
dove $T_m$ è il membership timeout configurato.
...
Quando un processo viene riavviato, mantiene la stessa \texttt{node\_id} ma incrementa la propria generation persistente. Tale generation viene utilizzata anche come incarnation della membership e come epoch dello stato. Di conseguenza, il nuovo processo può prevalere su informazioni o tombstone appartenenti alla precedente esecuzione.

I volumi Docker associati individualmente ai nodi preservano tale generation attraverso operazioni di stop/start.
...
La strategia di testing comprende test unitari e contrattuali, test concorrenti e in-memory, test black-box con socket reali e test d'integrazione mediante Docker Compose. Gli scenari includono convergenza, messaggi duplicati e fuori ordine, membership, crash/restart, leave, rejoin, partizioni temporanee e cluster a tre e sei nodi.

La pipeline di misura emette eventi strutturati \texttt{convergence\_sample}. Uno strumento separato raccoglie i log, ricostruisce le serie temporali e genera CSV e grafici SVG. La convergenza viene definita come il primo istante dal quale tutte le serie dei nodi rimangono entro una tolleranza assoluta di 0.05 rispetto all'oracle fino alla fine dei campioni disponibili.
...
La valutazione principale è stata effettuata sul cluster a sei nodi, considerato lo scenario più rappresentativo tra quelli inclusi nel progetto. Tutti i nodi utilizzano la stessa operazione di aggregazione durante una singola esecuzione.
...
Nelle esecuzioni attualmente registrate, tutte e quattro le aggregazioni hanno raggiunto il risultato atteso. La Tabella~\ref{tab:convergence} riporta il primo istante di convergenza stabile rilevato dalla pipeline.
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Aggregazione} & \textbf{Oracle} & \textbf{Tempo [s]} \\
\midrule
Average & 60  & \AvgConvTime \\
Sum     & 360 & \SumConvTime \\
Min     & 10  & \MinConvTime \\
Max     & 110 & \MaxConvTime \\
\bottomrule
...
In particolare, l'esecuzione più recente della media ha raggiunto convergenza stabile a circa \AvgConvTime~s. Gli altri esperimenti hanno mostrato convergenza a circa \SumConvTime~s per la somma, \MinConvTime~s per il minimo e \MaxConvTime~s per il massimo.

\IfFileExists{figures/convergence_average_6nodes.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/convergence_average_6nodes.pdf}
    \caption{Convergenza dell'aggregazione average nel cluster a sei nodi.}
    \label{fig:convergence-average}
\end{figure}
}{}

\IfFileExists{figures/convergence_sum_6nodes.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/convergence_sum_6nodes.pdf}
    \caption{Convergenza dell'aggregazione sum nel cluster a sei nodi.}
    \label{fig:convergence-sum}
\end{figure}
}{}

\IfFileExists{figures/convergence_min_6nodes.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/convergence_min_6nodes.pdf}
    \caption{Convergenza dell'aggregazione min nel cluster a sei nodi.}
    \label{fig:convergence-min}
\end{figure}
}{}

\IfFileExists{figures/convergence_max_6nodes.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/convergence_max_6nodes.pdf}
    \caption{Convergenza dell'aggregazione max nel cluster a sei nodi.}
    \label{fig:convergence-max}
\end{figure}
}{}
...
La fault tolerance è stata verificata arrestando progressivamente nodi del cluster e osservando l'evoluzione dell'aggregazione media. Il risultato iniziale con sei nodi è 60.

Dopo la failure detection del primo nodo arrestato, cinque contributi rimangono eleggibili e tutti i peer superstiti espongono una media pari a 50. Dopo un secondo arresto, quattro nodi risultano eleggibili e la media riconverge a 40. Riavviando progressivamente i processi, il sistema ritorna prima a cinque nodi e valore 50 e successivamente alla configurazione originale con sei nodi e valore 60.

\begin{table}[t]
\centering
\caption{Evoluzione della media durante crash e rejoin}
\label{tab:fault}
\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Fase} & \textbf{Nodi eleggibili} & \textbf{Media} \\
\midrule
Stato iniziale & 6 & 60 \\
Dopo primo crash & 5 & 50 \\
Dopo secondo crash & 4 & 40 \\
Primo rejoin & 5 & 50 \\
Riconvergenza finale & 6 & 60 \\
\bottomrule
\end{tabular}
\end{table}

L'esperimento evidenzia due proprietà differenti. Innanzitutto, l'indisponibilità di un nodo non arresta i round gossip dei peer superstiti. In secondo luogo, il valore aggregato è \emph{membership-aware}: dopo che la failure detection rende un contributo non eleggibile, il risultato viene ricalcolato esclusivamente sui nodi considerati attivi.

Al rejoin, la generation persistente del processo aumenta. La nuova incarnation prevale quindi sul precedente stato della membership e il contributo viene nuovamente incluso, consentendo il ritorno al valore iniziale.
...
Il jitter corrisponde al 20\% del delay configurato. \texttt{node-1} opera come riferimento senza NetEm, mentre gli altri nodi ricevono condizioni progressivamente più severe.

In una delle esecuzioni locali registrate per \emph{average}, la baseline senza Traffic Control convergeva stabilmente a circa \AvgConvTime~s. Con i profili NetEm attivi, tutti i nodi sono stati osservati sul valore corretto per la prima volta a circa \TCLocalFirstConv~s; l'osservazione è proseguita fino a \TCLocalStable~s per verificare che il risultato rimanesse stabile.

L'esperimento mostra quindi che l'introduzione di ritardi artificiali può aumentare il tempo necessario alla disseminazione completa dello stato senza impedire la convergenza.
...
La modalità Traffic Control è stata verificata anche in tale ambiente. Nell'esecuzione attualmente registrata con i profili definitivi 0--2000 ms, tutti i nodi hanno raggiunto l'oracle della media entro circa \TCAWSFirstConv~s e l'osservazione è proseguita fino a circa \TCAWSStable~s. La qdisc NetEm è stata verificata direttamente tramite \texttt{tc -s qdisc}, confermando l'applicazione del delay e del jitter configurati. Il controllo dei log dell'esecuzione definitiva non ha evidenziato transizioni spurie da \texttt{alive} a \texttt{suspect}.

Questo esperimento non viene utilizzato per sostenere che EC2 sia più veloce dell'ambiente locale: le due esecuzioni sono influenzate da scheduling, startup e risorse differenti. Il risultato è invece utilizzato come verifica della portabilità del deployment e della capacità del protocollo di convergere anche in presenza di delay sintetico su una macchina cloud Linux.

\IfFileExists{figures/baseline_vs_tc.pdf}{
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/baseline_vs_tc.pdf}
    \caption{Confronto tra convergenza baseline e Traffic Control.}
    \label{fig:baseline-tc}
\end{figure}
}{}
...
Gli esperimenti mostrano che la combinazione di gossip periodico, stato versionato e merge deterministico è sufficiente a portare i nodi allo stesso risultato nelle topologie considerate. La convergenza non dipende dall'esistenza di un processo coordinatore e la perdita temporanea di un peer non blocca la progressione dei round degli altri nodi.
...
I risultati di Traffic Control evidenziano che la convergenza viene mantenuta anche con delay e jitter significativamente superiori rispetto all'esecuzione baseline. L'esperimento deve tuttavia essere interpretato con cautela. NetEm introduce un delay egress sintetico all'interno di una topologia single-host e non riproduce completamente una WAN reale, nella quale intervengono routing, congestione, perdita correlata, variazioni dell'RTT e failure indipendenti delle macchine.

Anche la valutazione della scalabilità presenta limiti. I deployment effettivamente verificati comprendono tre e sei nodi, mentre test in-memory coprono topologie leggermente più grandi. Inoltre, nella configurazione principale a sei nodi il fanout è 5: ogni processo può quindi raggiungere tutti gli altri peer in un singolo round. I tempi osservati non devono essere generalizzati a cluster molto più grandi o a configurazioni con fanout significativamente inferiore.

Il costo di comunicazione per singolo round è limitato dal fanout, ma ciascun messaggio contiene sia informazioni aggregative sia un digest della membership. La dimensione dei payload e la memoria necessaria per conservare contributi versionati crescono pertanto con il numero di partecipanti. Una futura evoluzione potrebbe introdurre tecniche di compattazione o strutture di aggregazione con dimensione indipendente dal numero totale dei nodi.
...
Il sistema integra versionamento dello stato, deduplicazione, failure detection, membership dinamica e generation persistenti per gestire crash e rejoin. Le aggregazioni sono consapevoli dello stato della membership e utilizzano soltanto i contributi dei nodi considerati attivi.

Gli esperimenti su un cluster Docker Compose a sei nodi mostrano la convergenza ai quattro oracle attesi. Gli scenari di fault injection mostrano inoltre il ricalcolo dell'aggregato dopo la perdita di nodi e la successiva riconvergenza al loro rientro. Gli esperimenti NetEm dimostrano che il protocollo mantiene la convergenza anche con delay e jitter artificiali, mentre il deployment su AWS EC2 verifica la riproducibilità della soluzione nell'ambiente cloud previsto.

Il lavoro soddisfa quindi l'obiettivo di realizzare una computazione aggregativa decentralizzata e fault tolerant, lasciando come possibili sviluppi futuri una valutazione su cluster di dimensioni maggiori, la riduzione della dimensione dei messaggi e l'introduzione di meccanismi di sicurezza per la comunicazione tra peer.
