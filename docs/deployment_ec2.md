...
Lo script valida gli input prima di creare container, costruisce una volta l'immagine, ricrea un cluster pulito per run, verifica qdisc off/on tramite `scripts/experiments/traffic_control.sh`, raccoglie log/CSV/SVG/summary ed esegue cleanup tramite trap. Entrambe le condizioni usano `deploy/docker-compose.tc.yml` e i medesimi file `configs/traffic-control/node*.yaml`; cambia soltanto `TC_ENABLED`. `validate_run_evidence` rifiuta artefatti mancanti, summary incompleto, assenza per uno dei sei nodi di un CSV `local_round` positivo o `remote_merge` e transizioni `suspect`/`dead`; raccoglie inoltre `average_eligible_node_ids` in `average-eligibility.txt`, marca il primo raggiungimento dei sei ID e registra ogni regressione successiva sotto sei. Una regressione senza `suspect`/`dead` ferma l'esperimento e ne stampa le evidenze, senza workaround. `event=gossip_round` rimane diagnostico opzionale. Gli artefatti restano in `artifacts/traffic_control/<timestamp>/baseline/`, `delayed-<delay>/` e `comparison.txt`. Lo stato NetEm non viene dedotto dalla differenza dei tempi.
