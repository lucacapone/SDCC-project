...
Il contratto unico `validate_run_evidence` richiede `summary.txt`, `compose.log`, `convergence.csv`, `convergence.svg` e `qdisc.txt`; nel CSV richiede, per ciascuno di `node-1` … `node-6`, almeno un `local_round` con round strettamente positivo e almeno un `remote_merge`. Rifiuta inoltre ogni transizione `suspect`/`dead`. `event=gossip_round` resta un marker diagnostico opzionale: la sua assenza viene segnalata, ma non invalida una run già provata dal dataset canonico.
