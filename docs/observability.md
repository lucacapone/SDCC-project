...
Il contratto unico `validate_run_evidence` richiede `summary.txt`, `compose.log`, `convergence.csv`, `convergence.svg` e `qdisc.txt`; nel CSV richiede, per ciascuno di `node-1` … `node-6`, almeno un `local_round` con round strettamente positivo e almeno un `remote_merge`. Dal log produce anche `average-eligibility.txt`: vi conserva tutti i campioni `average_eligible_node_ids`, il primo campione contenente esattamente i sei ID canonici e le regressioni successive con meno di sei ID. Una regressione senza una transizione `suspect`/`dead` interrompe la validazione e viene stampata come evidenza diagnostica. Il criterio temporale canonico resta invariato. `event=gossip_round` resta un marker diagnostico opzionale: la sua assenza viene segnalata, ma non invalida una run già provata dal dataset canonico.

`qdisc.txt` conserva la fotografia separata di NetEm, mentre lo stato off/on viene asserito a caldo esclusivamente tramite `scripts/experiments/traffic_control.sh`; non viene mai dedotto dai tempi di convergenza né mediante parsing sostitutivo degli artefatti. Essendo un qdisc egress, il delay riguarda i datagrammi UDP gossip inviati.
