...
`deploy/docker-compose.tc.yml` riproduce i sei servizi, monta `configs/traffic-control/node1.yaml` … `configs/traffic-control/node6.yaml` e usa la rete `sdcc-net` della variante scale. Queste configurazioni replicano senza variazioni i parametri runtime canonici, incluso `membership_timeout_ms: 10000`; soltanto la discovery seed e' completa e simmetrica verso gli altri cinque nodi. A differenza del Compose canonico usa `deploy/traffic-control/Dockerfile`, una shell con `iproute2`/`su-exec` e aggiunge **soltanto** `NET_ADMIN`; non usa `privileged: true`. L'entrypoint applica o rimuove NetEm e avvia lo stesso `/usr/local/bin/sdcc-node` come UID/GID `65532`.
...
Durante il confronto, baseline e delayed usano esattamente lo stesso Compose e gli stessi mount; l'unica variabile di condizione e' `TC_ENABLED`. `validate_run_evidence <run_dir> baseline|delayed` centralizza la validazione: richiede summary completo e gli artefatti della run, campioni CSV `local_round` con round positivo e `remote_merge` per tutti i sei nodi e nessuna transizione `suspect`/`dead`. `average-eligibility.txt` raccoglie ogni `average_eligible_node_ids`, individua il primo insieme completo `node-1` … `node-6` e riporta le successive regressioni sotto sei; una regressione senza `suspect`/`dead` interrompe la prova conservando le righe diagnostiche. Il marker log `event=gossip_round` è diagnostico opzionale. L'asserzione del qdisc resta demandata a `scripts/experiments/traffic_control.sh assert-off|assert-on`; i tempi non sono usati come prova dell'attivazione di NetEm.
